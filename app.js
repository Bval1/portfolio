...
window.addEventListener("scroll", () => {
  ...
  authorName.style.transform = `translateX(calc(-100vh + ${offsetY * 0.9}px))`;
  
  jobTitleContainer[0].style.backgroundPositionY = `${offsetY * 0.5}px`;
  jobTitle[0].style.transform = `translateX(calc(200vh - ${offsetY}px))`; // bring it back to 0 point

  projectTitle.style.transform = `translateY(calc(300vh - ${offsetY}px))`
  phones[0].style.transform = `translateX(calc(390vh - ${offsetY}px))`
  laptop.style.transform = `translateX(calc(-480vh + ${offsetY}px))`
  phones[1].style.transform = `translateX(calc(590vh - ${offsetY}px))`
})